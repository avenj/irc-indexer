...
This is the data returned by L<IRC::Indexer::Info::Network> after it has 
been fed at least one L<IRC::Indexer::Info::Server>.
...
As of IRC::Indexer-0.01, the only per-server key is B<MOTD>, containing 
the array of lines composing the server's MOTD (without newlines).
